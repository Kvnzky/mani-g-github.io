...
<body class="min-h-screen">
  ...
  <script type="text/babel">
    ...
    function App() {
      ...
      const syncCloudSettings = (overrides = {}) => {
        if (!appsScriptUrl) return;
        const nextProducts = overrides.products || products;
        const nextPaymentMethods = overrides.paymentMethods || paymentMethods;
        const nextQrs = overrides.customQrs || customQrs;
        const availMap = {};
        nextProducts.forEach(p => { availMap[p.id] = p.available !== false; });
        ...
        fetch(appsScriptUrl, {
          ...
          body: JSON.stringify({
            ...
            settings: {
              cutoff: {
                enabled: true,
                date: getPhilippineDateTime().dateStr,
                time: '23:59',
                paymentMethods: nextPaymentMethods,
                flavorAvailability: availMap,
              ...
              paymentMethods: nextPaymentMethods,
              flavorAvailability: availMap,
              qrs: { gcashNumber: nextQrs.gcashNumber || GCASH_NUMBER }
            }
          })
        }).catch(() => {});
      };
      ...
      useEffect(() => {
        ...
        const pollCloud = async () => {
          try {
            const res = await fetch(\`\${appsScriptUrl}?action=getSettings&_t=\${Date.now()}\`, { cache: 'no-store' });
            if (res.ok) {
              const data = await res.json();
              const s = data && data.settings ? data.settings : null;
              if (s) {
                const pm = s.paymentMethods || (s.cutoff && s.cutoff.paymentMethods);
                if (pm && typeof pm === 'object') {
                  setPaymentMethods({
                    cod: pm.cod !== undefined ? Boolean(pm.cod) : true,
                    maribank: pm.maribank !== undefined ? Boolean(pm.maribank) : true,
                    gcash: pm.gcash !== undefined ? Boolean(pm.gcash) : true
                  });
                }
                if (Array.isArray(s.products) && s.products.length > 0) {
                  setProducts(prev => prev.map(defP => {
                    const cp = s.products.find(x => x.id === defP.id);
                    return cp ? { ...defP, ...cp, image: defP.image } : defP;
                  }));
                }
                const gNum = (s.qrs && s.qrs.gcashNumber) || (s.cutoff && s.cutoff.gcashNumber);
                if (gNum) {
                  setCustomQrs(prev => ({ ...prev, gcashNumber: String(gNum).trim() }));
                }
              }
            }
          } catch (e) {}
      ...
      const handleFileUpload = (e, qrKey) => {
        ...
        reader.onload = () => {
          setCustomQrs(prev => {
            const next = { ...prev, [qrKey]: reader.result };
            syncCloudSettings({ customQrs: next });
            return next;
          });
              ...
              <div className="space-y-6 animate-fade-in">
                ...
                <div className="bg-white rounded-3xl p-5 border border-mani-200 shadow-sm space-y-4">
                  <div className="flex justify-between items-center border-b border-mani-100 pb-2">
                    ...
                    <button
                      onClick={() => {
                        const nextPm = { cod: true, maribank: true, gcash: true };
                        setPaymentMethods(nextPm);
                        syncCloudSettings({ paymentMethods: nextPm });
                    ...
                    <div className="p-3 rounded-2xl border flex items-center justify-between bg-orange-50/50">
                      ...
                      <button
                        onClick={() => {
                          ...
                          setPaymentMethods(prev => {
                            const nextPm = { ...prev, maribank: nextVal };
                            syncCloudSettings({ paymentMethods: nextPm });
                            return nextPm;
                          });
                        }}
                    ...
                    <div className="p-3 rounded-2xl border flex items-center justify-between bg-blue-50/50">
                      ...
                      <button
                        onClick={() => {
                          ...
                          setPaymentMethods(prev => {
                            const nextPm = { ...prev, gcash: nextVal };
                            syncCloudSettings({ paymentMethods: nextPm });
                            return nextPm;
                          });
                        }}
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 text-xs">
                    ...
                    <div className="p-3.5 rounded-2xl bg-blue-50/70 border border-blue-200 space-y-2 text-center">
                      ...
                      <div className="flex gap-1.5 pt-1">
                        ...
                        <button
                          onClick={() => {
                            syncCloudSettings({ customQrs });
                            setQrSaveMsg('GCash number saved & synced!');
                            setTimeout(() => setQrSaveMsg(''), 2500);
                          }}
                        ...
                        <div key={p.id} className="p-2.5 rounded-xl border flex items-center justify-between gap-2 bg-mani-50/50">
                          ...
                          <button
                            onClick={() => {
                              ...
                              setProducts(updated);
                              syncCloudSettings({ products: updated });
                            }}
